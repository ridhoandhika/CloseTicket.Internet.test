...
$(function() {
	...
	// $('#refreshookla').click(function() {
	// 	// document.location.reload();
	// 	$('#ookla').attr("src", $('#ookla').attr("src"));
	// 	$('#submit-speed').addClass('disabled');
	// 	// document.getElementById(ookla).contentDocument.location.reload(true);
	// 	$('#refreshookla').prop("disabled", true);
	// });
...
function retrievepassed(){
	$.ajax({
		method: 'post',
		dataType: 'json',
		url: site_url('Check_devembed2/retrieveSpeed'),
		contentType: 'application/json',
		data: {},
...
function ooklaListener(ev) {
	...
	if(flagcontinue == true){
		...
		let checkreport	= setInterval(function() {
			// console.log(ev.data);
			if(flagcontinue == true){
				$('#submit-speed').trigger('click');
...
function doTask() {
	...
	$.ajax({
		method: 'post',
		dataType: 'json',
		url: site_url('Check_devembed2/createTask'),
		data: {
...
function doToken1() {
	$.ajax({
		method: 'post',
		dataType: 'json',
		url: site_url('Check_devembed2/retrieveToken1'),
		data: {},
...
function doIPRadius() {
	...
	$.ajax({
		method: 'post',
		dataType: 'json',
		url: site_url('Check_devembed2/retrieveIPRadius'),
		data: {},
			...
			success: function(response3) {
				...
				if(typeof response3.ip_addr != 'undefined'){
					...
					$.ajax({
						method: 'post',
						dataType: 'json',
						url: site_url('Check_devembed2/saveWhoIP4'),
						data: {
							ip_addr: ip_addr,
							success: 1,
							request: null,
...
function doOokla(){
	$.ajax({
		method: 'post',
		dataType: 'json',
		url: site_url('Check_devembed2/retrieveSpeedOokla'),
		data: {},
...
function doSpeedAcsis() {
	...
	$.ajax({
		method: 'post',
		dataType: 'json',
		url: site_url('Check_devembed2/retrieveSpeedAcsis'),
		data: {},
...
function doRedaman() {
	...
	$.ajax({
		method: 'post',
		dataType: 'json',
		url: site_url('Check_devembed2/retrieveUkur'),
		data: {},
...
function doSpeed() {
	...
	$.ajax({
		method: 'post',
		dataType: 'json',
		url: site_url('Check_devembed2/retrieveToken2'),
		data: {},
		success: function(response8) {
			if(response8.success) {
				$.ajax({
					method: 'post',
					dataType: 'json',
					url: site_url('Check_devembed2/retrievePCRF'),
					data: {},
...
function sendAutoClose(){
	$.ajax({
		method: 'post',
		dataType: 'json',
		url: site_url('Check_devembed2/retrieveCloseAuto'),
		data: {
			// close: close
		},
