...
class Dashboard extends CI_Controller {
	...
	public function index()	{
		...
		if($this->session->userdata('SignedIn') == true) {
			...
			header('Location: https://'.$_SERVER['HTTP_HOST'].'/CloseTicket.Internet.test/Dashboard/pageSignIn');
		}
	...
	public function pageSignOut() {
		...
		//redirect('Dashboard/index');
		header('Location: https://'.$_SERVER['HTTP_HOST'].'/CloseTicket.Internet.test/Dashboard/');
			...
			if($this->input->post('filter-start') != '' && $this->input->post('filter-finish') != '') {
				$q01 = $this->db->query('
					SELECT
						...
						ip_success,
						ip_failed,
						validasi_success_passed_1,
						validasi_success_passed_0,
						validasi_failed,
						token_1_success,
						token_1_failed,
						nd_success_nossa,
	...
	public function jOverviewDetail()	{
		...
		if($this->session->userdata('SignedIn') == true) {
			if($this->input->post('detail-type') != '' && $this->input->post('detail-tick') != '') {
				...
				switch($this->input->post('detail-field')) {
					...
					case 'ip_failed':
						$w01 = '
							AND bearer1 IS NOT NULL
							AND nd IS NOT NULL
							AND frame_ip IS NULL 
						';
						$token++;
						break;
					case 'validasi_success_passed_1':
						$w01 = '
							AND bearer1 IS NOT NULL
							AND bearer1 <> \'\'
							AND nd IS NOT NULL
						...
						$token++;
						break;
					case 'validasi_success_passed_0':
						$w01 = '
							AND bearer1 IS NOT NULL
							AND bearer1 <> \'\'
							AND nd IS NOT NULL
						...
						$token++;
						break;
					case 'validasi_failed':
						$w01 = '
							AND bearer1 IS NOT NULL
							AND bearer1 <> \'\'
							AND nd IS NOT NULL
							AND frame_ip IS NOT NULL
							AND (ip_addr IS NULL OR ip_addr = \'\')
							AND ip_passed is null							
						';
						$token++;
						break;
				...
				//CASE WHEN frame_ipv6 IS NOT NULL THEN CASE WHEN frame_ipv6 IS NOT NULL THEN
				if($token > 0) {
					if($this->input->post('detail-type') == 'hour') {
						$q01 = $this->db->query('
							SELECT
								ts,
								TIME(ts_finish) AS finish,
								x.uid as uid_scc,
								x.nd as nd_scc,
								remote_addr,
								bras,
							...
							FROM
								task_archive x
							LEFT JOIN 
							(select distinct uid, ts_start, response from log_getResult where uid is not null AND name_api="getResultSCCLog" and date(ts_start) >=  date(now()) - interval 7 day order by ts_start ASC) y on y.uid=x.uid
							WHERE
								1 = 1
		...
		if($this->session->userdata('SignedIn') == true) {
			if($this->input->post('ticket') != '') {
				$q01 = $this->db->query('
					...
					Group by x.uid
					ORDER BY
						ts DESC,ts_start DESC
				');
				$output['success'] = true;
				$output['data'] = array(
					'detail' => $q01->result_array()
				); //	(SELECT uid, response,ts_start FROM log_getResult where uid is not null order by ts_start ASC limit 20000) y on y.uid=x.uid
			}
			else {
				$error[] = 'empty ticket';
			}
