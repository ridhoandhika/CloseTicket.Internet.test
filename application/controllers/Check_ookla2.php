...
class Check_ookla2 extends CI_Controller {
	...
	public function retrieveCloseAuto()	{
		...
		if($token > 0 || !$this->whitelistEnabled) {
			...
			$r01 = $q01->row_array();
			if($r01['redaman_passed'] == 1 && $r01['speed_passed'] == 1){
				$passed = 1;
			} 
	...
	public function getResultSCCdev3()	{
		$datestart = date('Y-m-d H:i:s');
		$output = array(
			'success' => false
		);
	...
	public function getResultSCC()	{
		...
		$tLog = time().'.'.rand(111111, 999999);
		if(empty($request['nd'])){
			$logRequest = $this->ModelLog->writelog($tLog.'.getresultscc.request.log.'.$requestticket['ticket'], json_encode($requestticket));
			$fileRequest = null;
		}else{	
			$logRequest = $this->ModelLog->writelog($tLog.'.getresultscc.request.log.'.$request['nd'], json_encode($request));
			$fileRequest = null;
		}
	    if($logRequest['success']) {
				$fileRequest = $logRequest['data']['file'];
		...
		if(
			($this->input->post('ticket') != '' 
			&&  $this->input->post('hash') != ''
			&& sha1('INFOMEDIA#'.$this->input->post('ticket').'#NUSANTARA') == $this->input->post('hash'))
			|| ($this->input->post('nd') != ''
			&& $this->input->post('hash') != ''
			&& sha1('INFOMEDIA#'.$this->input->post('nd').'#NUSANTARA') == $this->input->post('hash'))				
		) {
		...
		//log response
		if(empty($request['nd'])){
			$logResponse = $this->ModelLog->writelog($tLog.'.getresult.response.log.'.$requestticket['ticket'], json_encode($output));
			$fileResponse = null;
		}else{	
			$logResponse = $this->ModelLog->writelog($tLog.'.getresult.response.log.'.$request['nd'], json_encode($output));
			$fileResponse = null;
		}
		if($logResponse['success']) {
			$fileResponse = $logResponse['data']['file'];
