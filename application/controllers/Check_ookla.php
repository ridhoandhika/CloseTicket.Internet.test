...
class Check_ookla extends CI_Controller {
	...
	public function saveWhoIP6()	{		
		...
		if($token > 0 || !$this->whitelistEnabled) {
			...
			if($this->form_validation->run() == false) {
				...
				//ip_nas = '.$this->db->escape($this->input->post('ip_nas')).',
				$q01 = $this->db->query('
				...
				if(count($ndEx) > 1) {
					...
					$q01 = $this->db->query('
						UPDATE task_active SET
							...
							redaman_ts = NOW(),
							ip_nas = '.$this->db->escape($v01nasip).',
	...
	public function retrieveSpeedOokla() {
		...
		if($token > 0 || !$this->whitelistEnabled) {
			$q01 = $this->db->query('
				...
				FROM
					task_active a
				INNER JOIN mapping_ookla b on a.ip_nas=b.ip_nas
				WHERE
					1 = 1
					AND uid = '.$this->db->escape($this->session->userdata('uid')).'
			');
