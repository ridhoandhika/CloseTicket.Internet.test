...
class Check_embeded extends CI_Controller {
	...
	function index(){
		...
		*/
		...
		if($token > 0 || !$this->whitelistEnabled) {
			...
			// $this->session->set_userdata('ticket', $tick);
			// // var_dump($this->session->set_userdata('ticket', $ticket));
			// $this->session->set_userdata('nd', $nde);
			$this->session->set_userdata('uid', '');
			$this->ModelTemplate->render();
		}
		else {
			// redirect('Welcome');
			header('Location: https://'.$_SERVER['HTTP_HOST'].'/CloseTicket.Internet/Welcome');
		}
	...
	public function createTask()	{
		...
		if($token > 0 || !$this->whitelistEnabled) {
			...
			if($this->input->post('nd') !=''){
				$nd = $this->input->post('nd').'@telkom.net';
				$nd_number = $this->input->post('nd');
			}else {
				$nd =  null;
				$nd_number = null;
			}
			$request = array(
				...
				'nd' => $nd_number// $this->input->post('nd')
			);
			$tLog = time().'.'.rand(111111, 999999);
			$logRequest = $this->ModelLog->write($tLog.'.submit.request.log', json_encode($request));
			$fileRequest = null;
			...
			$q02 = $this->db->query('
				...
				WHERE
					1 = 1
					AND ip_bras = '.$this->db->escape($qrange).'
				limit 1;
			');	
			$r01 = $q02->row_array();
			...
			if($this->form_validation->run() == false) {
				...
				$q01 = $this->db->query('
					...
					VALUES (
						...
						'.$this->db->escape($this->remoteIP).',
						'.$this->db->escape($qrange).',
						'.$this->db->escape($r01['bras']).',
						'.$this->db->escape($r01['speedtest']).',
						'.$this->db->escape($this->input->post('ticket')).',
						'.$this->db->escape($fileRequest).',
						'.$this->db->escape($nd_number).',
						'.$this->db->escape($nd).',
						\'Sonic\',
						\''.$datestart.'\'
					)
		...
		if($token > 0 || !$this->whitelistEnabled) {
			...
			$q01 = $this->db->query('
				UPDATE task_active SET
					...
					token1_ts = '.$this->db->escape($datestart).',
					bearer1 = '.$this->db->escape($v01Bearer).',
					ts_finish = '.$this->db->escape($datestart).'
				WHERE
					1 = 1
	...
	public function retrieveIPRadius() {
		$output = array(
			'success' => false
		);
		$error = array();
		...
		if($token > 0 || !$this->whitelistEnabled) {
            ...
            for($iRealm = 0; $iRealm < 3; $iRealm++) {
                ...
                if(count($ndEx) > 1) {
                    for($iRetry = 0; $iRetry < 5; $iRetry++) {
                        ...
                        $soCh = curl_init();
                        curl_setopt($soCh, CURLOPT_URL, 'https://apigw.telkom.co.id:7777/gateway/telkom-radius/1.0/getUsageOnline');
                        curl_setopt($soCh, CURLOPT_POST, TRUE);
                        ...
                        $tLog = time().'.'.rand(111111, 999999);
                        $logRequest = $this->ModelLog->write($tLog.'.ipradius.request.log', $soBody);
                        $fileRequest = null;
                        if($logRequest['success']) {
                            $fileRequest = $logRequest['data']['file'];
                        }
                        $logResponse = $this->ModelLog->write($tLog.'.ipradius.response.log', $soResponse.PHP_EOL.'========================='.PHP_EOL.$soError);
                        $fileResponse = null;
                        if($logResponse['success']) {
                            $fileResponse = $logResponse['data']['file'];
                        }
                        ...
                        if($soError == '') {
                            ...
                            if(isset($jipradius['returnMessage']) == 'OK'){
                                if((isset($jipradius['output']['Delegated-IPv6-Prefix'])) && (isset($jipradius['output']['Framed-IP-Address']))){
                                    $frameip = $jipradius['output']['Framed-IP-Address'];
                                    $frameipv6 = $jipradius['output']['Delegated-IPv6-Prefix'];
                                }else if(isset($jipradius['output']['Delegated-IPv6-Prefix'])){
                                    $frameip = $jipradius['output']['Delegated-IPv6-Prefix'];
                                    $frameipv6 =  null;
                                }else if(isset($jipradius['output']['Framed-IP-Address'])){
                                    $frameip = $jipradius['output']['Framed-IP-Address'];
                                    $frameipv6 = null;
                                }
			...
			if(isset($frameip)){
				$v01frame_ip = $frameip;
				$v01frame_ip6 = $frameipv6;
				$nde = $ndEx[0].'@'.$realm[$iRealm];

				$output['success'] = true;
				$output['data'] = array(
					'frame_ip' => $v01frame_ip,
					'frame_ipv6' => $v01frame_ip6,
					'retry' => $iRealm
				);
			
			}else {
				$output['error'] = $soResponse;
				$nde = $r01['nd'];
				$v01frame_ip = 0;
			}
			...
			$q01 = $this->db->query('
				UPDATE task_active SET
					retry = '.$this->db->escape($v01retry).',
					nd = '.$this->db->escape($nde).',
					ipbynd_succes = '.$this->db->escape($chSuccess).',
					ipbynd_log_request = '.$this->db->escape($fileRequest).',
					ipbynd_log_response = '.$this->db->escape($fileResponse).',
					ipbynd_log_total_time = '.$this->db->escape($total_time).',
					ipbynd_ts = '.$this->db->escape($datestart).',
					frame_ip = '.$this->db->escape($v01frame_ip).',
					frame_ipv6 = '.$this->db->escape($v01frame_ip6).',
					ts_finish = '.$this->db->escape($datestart).'
				WHERE
					1 = 1
					AND uid = '.$this->db->escape($this->session->userdata('uid')).'
			');
			if(isset($frameip) == null) {
				$q02 = $this->db->query('
					DELETE FROM task_active
					WHERE
						1 = 1
	...
	public function saveWho(){
		...
		if($token > 0 || !$this->whitelistEnabled) {
			...
			$r01 = $q02->row_array();
			if(isset($r01)){
				$frameip = $r01['frame_ip'];
				$frameipv6 = $r01['frame_ipv6'];
			}else {
				$frameip = null;
				$frameipv6 = null;
			}
			$ip_addr = $this->input->post('ip_addr');
			if(isset($ip_addr) != null ){
				if(isset($frameip) != null || isset($frameipv6) != null){
					if($ip_addr == $frameip ||  $ip_addr == $frameipv6 ) {
						$passed = 1;
					}else {
						$passed = 1;
					}
			...
			if($this->form_validation->run() == false) {
				...
				$q01 = $this->db->query('
					UPDATE task_active SET
						...
						who_ts = '.$this->db->escape($datestart).',
						ip_addr = '.$this->db->escape($this->input->post('ip_addr')).',
						ip_passed = '.$this->db->escape($passedIP).',
						ts_finish ='.$this->db->escape($datestart).'
					WHERE
						1 = 1
						AND uid =  '.$this->db->escape($this->session->userdata('uid')).'
			...
			if($q01->num_rows() > 0) {
				...
				if(count($ndEx) > 1) {
					...
					for($iRetry = 0; $iRetry < 5; $iRetry++) {
						...
						curl_close($soCh);
						$Ukurindikasi = json_decode($soResponse, true);
						if(isset($Ukurindikasi['MESSAGE'])){
							$messages =  $soResponse;//$Ukurindikasi['MESSAGE'];
						} else {
							if(!isset($Ukurindikasi['onu_rx_pwr'])){
								$messages = $soResponse;
							}else {
								$messages = null;
							}
					...
					$q01 = $this->db->query('
						UPDATE task_active SET
							...
							redaman_message = '.$this->db->escape($v02message).',
							redaman_ts = '.$this->db->escape($datestart).',
		...
		if($token > 0 || !$this->whitelistEnabled) {
			...
			$q01 = $this->db->query('
				UPDATE task_active SET
					...
					token2_ts = '.$this->db->escape($datestart).',
		...
		if($token > 0 || !$this->whitelistEnabled) {
			...
			$q01 = $this->db->query('
					UPDATE task_active SET
						...
						pcrf_ts = '.$this->db->escape($datestart).',
	...
	public function SaveSpeed(){
		...
		if($token > 0 || !$this->whitelistEnabled) {
			...
			$serverOokla = null;
			if(isset($payload)){
				if($payload['config']['serverId'] == 37343){
					$serverOokla = 'JAKARTA';
				}else if($payload['config']['serverId'] == 45466){
					$serverOokla = 'MEDAN';
				}else if($payload['config']['serverId'] == 37344){
					$serverOokla = 'SURABAYA';
				}else if($payload['config']['serverId'] == 45462){
					$serverOokla = 'BANDUNG';
				}else if($payload['config']['serverId'] == 45463){
					$serverOokla = 'SEMARANG';
				}else if($payload['config']['serverId'] == 45464){
					$serverOokla = 'BALIKPAPAN';
				}else if($payload['config']['serverId'] == 45465){
					$serverOokla = 'MAKASAR';
				}else{
					$serverOokla = null;
				}
			...
			$v01speedtest_server = $serverOokla;// $payload['config']['serverId'];
			...
			if(isset($v01speedtest_download)) {
				$output['success'] = true;
				$output['data'] = array(
					'speed' => $v01speedtest_download
					// 'message' => $v01message
				);
			}
			...
			$this->db->query('
				...
				serverOokla = '.$this->db->escape($v01speedtest_server).',
			...
			$q01a = $this->db->query('
				...
				VALUES (
					...
					'.$this->db->escape($v01speedtest_server).',
					'.$this->db->escape($v01speedtest_download).',
					'.$this->db->escape($v01speedtest_upload).',
					'.$this->db->escape($v01speedtest_units).',
					'.$this->db->escape($v01speedtest_latency_minimum).',
					'.$this->db->escape($v01speedtest_latency_jitter).',
					'.$this->db->escape($datestart).'
				)
			');
	...
	public function retrieveSpeed() {
		...
		if($token > 0 || !$this->whitelistEnabled) {
			...
			if(isset($q01)){
				if($q01->num_rows() > 0) {
					$r01 = $q01->row_array();
					$download = $r01['speedtest_download'];
					...
					if($checkDevice && $checkQuota) {
						$chSuccess = 0;
						if(isset($download) && isset($download) != null) {
							$speed = round($download / 1.024) / 1000;
							$q02 = $this->db->query('
								SELECT
									speed_requirement(
			...
			$q01c = $this->db->query('
					UPDATE task_active SET
						...
						speed_ts = '.$this->db->escape($datestart).',
	...
	public function retrieveCloseAuto()	{
		$output = array(
			'success' => false
		);
		$error = array();
		...
		if($token > 0 || !$this->whitelistEnabled) {
			...
			$r01 = $q01->row_array();
			if(isset($r01)){
				if($r01['redaman_passed'] == null || $r01['speed_passed'] == null ){
					$passed = null;
				}else if($r01['redaman_passed'] == 1 && $r01['speed_passed'] == 1){
					$passed = 1;
				} 
				else{
					$passed = 0;
			...
			$v01passed= $passed;
			if($q01->num_rows() > 0) {
				if(isset($v01passed) != null){
					$this->db->query('
						UPDATE task_active
						SET
							close = '.$v01passed.',
							ts_finish = '.$this->db->escape($datefinish).',
							incomplete = 0
						WHERE
							1 = 1
							AND uid =  '.$this->db->escape($this->session->userdata('uid')).'
	...
	public function retrieveCloseAuto_old()	{
		date_default_timezone_set('Asia/Jakarta');
		...
		if($token > 0 || !$this->whitelistEnabled) {
			...
			$r01 = $q01->row_array();
			if(isset($r01['redaman_passed']) == null || isset($r01['speed_passed']) == null ){
				$passed = null;
			}else if(isset($r01['redaman_passed']) == 1 && isset($r01['speed_passed']) == 1){
				$passed = 1;
			} 
			else if(isset($r01['redaman_passed']) == 0 || isset($r01['speed_passed']) == 0){
				$passed = 0;
			}
			...
			if($q01->num_rows() > 0) {
				if(isset($v01passed) != null){
					$this->db->query('
						UPDATE task_active
						SET
							close = '.$this->db->escape($v01passed).',
							ts_finish = '.$this->db->escape($datestart).',
							incomplete = 0
						WHERE
							1 = 1
					...
					$q02b = $this->db->query('
						DELETE FROM task_active
						WHERE
							uid =  '.$this->db->escape($this->session->userdata('uid')).'
					');
				}else {
					$output['success'] = true;
					$output['data'] = array(
	...
	public function getResultSCCLog()	{
		date_default_timezone_set('Asia/Jakarta');
		...
		$tLog = time().'.'.rand(111111, 999999);
		$logRequest = $this->ModelLog->writelog($tLog.'.getresultscc.request.log.'.$request['ticket'], json_encode($request));
		$fileRequest = null;
		...
		if(
			...
			if($q01->num_rows() > 0) {
				$r01 = $q01->row_array();
				if($r01['redaman_message'] == null || $r01['redaman_message'] == '' ){
					$output['success'] = true;
					$output['data'] = array(
						'ticket' => $r01['ticket'],
						'ip_addr' => $r01['ip_addr'],
					...
					$output['data'] = array(
						...
						'ip_addr' => $r01['ip_addr'],
						'nd' => $r01['nd'],
						'redaman_message' => $r01['redaman_message'],
						'redaman_passed' => $r01['redaman_passed'],
						'speed_passed' => $r01['speed_passed'],
						'close' => $r01['close']
					);	
				...
				if($q03->num_rows() > 0) {
					$r03 = $q03->row_array();
					if($r03['redaman_message'] == null || $r03['redaman_message'] == '' ){
						$output['success'] = true;
						$output['data'] = array(
							'ticket' => $r03['ticket'],
							'ip_addr' => $r03['ip_addr'],
							'nd' => $r03['nd'],
							'redaman_passed' => $r03['redaman_passed'],
							'speed_passed' => $r03['speed_passed'],
							'close' => $r03['close']
						);
					}
					else {
						...
						$output['data'] = array(
							...
							'ip_addr' => $r03['ip_addr'],
							'nd' => $r03['nd'],
							'redaman_message' => $r03['redaman_message'],
							'redaman_passed' => $r03['redaman_passed'],
							'speed_passed' => $r03['speed_passed'],
							'close' => $r03['close']
						);
		...
		$logResponse = $this->ModelLog->writelog($tLog.'.getresult.response.log.'.$request['ticket'], json_encode($output));
		$fileResponse = null;
