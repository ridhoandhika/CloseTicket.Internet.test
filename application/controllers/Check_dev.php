...
class Check_dev extends CI_Controller {
		...
		if($token > 0 || !$this->whitelistEnabled) {
			...
			if($q01->num_rows() > 0) {
				...
				for($iRetry = 0; $iRetry < 5; $iRetry++) {
					...
					$soCh = curl_init();
					curl_setopt($soCh, CURLOPT_URL, 'https://apigwsit.telkom.co.id:7777/ws/telkom-mycx-tcIncident/1.0/QueryTK_INCIDENT');
					curl_setopt($soCh, CURLOPT_POST, TRUE);
	...
	public function  retrieveIPRadius() {
		...
		if(isset($frameip) != null || isset($frameipv6) != null){
			if($frameip == $r01['ip_addr'] || $frameipv6 == $r01['ip_addr']) {
				$passed = 1;
			}else {
				$passed = 0;
			}
