...
class Check extends CI_Controller {
	...
	public function index()	{
		...
		*/
		...
		if($token > 0 || !$this->whitelistEnabled) {
			...
			// redirect('Welcome');
			header('Location: https://'.$_SERVER['HTTP_HOST'].'/CloseTicket.Internet.test/Welcome');
		}
		...
		if($token > 0 || !$this->whitelistEnabled) {
			...
			$q01 = $this->db->query('
				UPDATE task_active SET
					...
					token1_ts = '.$this->db->escape($datefinish).',
	...
	public function retrieveNDByIN()	{
		$output = array(
			'success' => false
		);
		$error = array();
		...
		if($token > 0 || !$this->whitelistEnabled) {
			...
			if($q01->num_rows() > 0) {
				...
				for($iRetry = 0; $iRetry < 5; $iRetry++) {
					...
					$tLog = time().'.'.rand(111111, 999999);
					$logRequest = $this->ModelLog->write($tLog.'.ndbyin.request.log', $soBody);
					$fileRequest = null;
					if($logRequest['success']) {
						$fileRequest = $logRequest['data']['file'];
					}
					$logResponse = $this->ModelLog->write($tLog.'.ndbyin.response.log', $soResponse.PHP_EOL.'========================='.PHP_EOL.$soError);
					$fileResponse = null;
					if($logResponse['success']) {
						$fileResponse = $logResponse['data']['file'];
					}
					curl_close($soCh);
					$ndbyin = json_decode($soResponse, true);
					if(!isset($ndbyin['QueryTK_INCIDENTResponse']['TK_INCIDENTSet']['INCIDENT']['TK_SERVICENO'])){
					...
					if($soError == '') {
						$chSuccess = 1;
						try {
							$jINNossa = json_decode($soResponse, true);
						}
						catch(Exception $ex) {
						...
						if(
							isset($jINNossa['QueryTK_INCIDENTResponse']['TK_INCIDENTSet']['INCIDENT']['TK_SERVICENO'])
						) {
							$nd_number = $jINNossa['QueryTK_INCIDENTResponse']['TK_INCIDENTSet']['INCIDENT']['TK_SERVICENO'];
						}else {
							$nd_number= null;
						}
					}
				...
				if(isset($nd_number) != null ){
					$result_nd = preg_replace("/[^0-9]/", "", $nd_number);
					if($result_nd == "" ){
						$result_nd = null;
					}
					$nd= $nd_number.'@telkom.net';
				}else {
					$nd = null;
					$result_nd = null;
				}
				...
				$q01 = $this->db->query('
					UPDATE task_active SET
						...
						ndbyip_ts ='.$this->db->escape($datefinish).',
						nd = '.$this->db->escape($nd).',
						nd_number = '.$this->db->escape($result_nd).',
						ts_finish = '.$this->db->escape($datefinish).'
					WHERE
						1 = 1
						AND uid = '.$this->db->escape($this->session->userdata('uid')).'
				');
				if(!isset($nd_number) || $nd_number == null) {
					$q02 = $this->db->query('
						DELETE FROM task_active
						WHERE
							1 = 1
	...
	public function retrieveNDByIP_old()	{
			...
			if($q01->num_rows() > 0) {
				...
				for($iRealm = 0; $iRealm < 1; $iRealm++) {
					...
					if(count($ndEx) > 1) {
						for($iRetry = 0; $iRetry < 5; $iRetry++) {
							...
							$tLog = time().'.'.rand(111111, 999999);
							$logRequest = $this->ModelLog->write($tLog.'.pcrf.request.log', $soBody.PHP_EOL.'========================='.PHP_EOL.$r01['bearer1']);
							$fileRequest = null;
							if($logRequest['success']) {
								$fileRequest = $logRequest['data']['file'];
							}
				...
				if($packageName != null){
					$q03 = $this->db->query('
						...
						FROM 
							mapping_speedtest
						WHERE 
							package_name = '.$this->db->escape($packageName).'
						GROUP BY 
							package_name
						limit 1;
					');
					// var_dump($q03);
					// '.$this->db->escape($packageName).'
					$r03 = $q03->row_array();
				...
				$q01 = $this->db->query('
					UPDATE task_active SET
						...
						pcrf_ts = '.$this->db->escape($datefinish).',
		...
		if($token > 0 || !$this->whitelistEnabled) {
			...
			$q01 = $this->db->query('
				UPDATE task_active SET
					...
					realm = '.$this->db->escape($v01real_nd).',
	...
	public function  retrieveIPRadius_old() {
		$output = array(
			'success' => false
		);
		$error = array();
		...
		$q01 = $this->db->query('
				UPDATE task_active SET
					...
					ipbynd_log_total_time = '.$this->db->escape($total_time).',
					ipbynd_ts = '.$this->db->escape($datestart).',
					frame_ip = '.$this->db->escape($frameip).',
					frame_ipv6 = '.$this->db->escape($frameipv6).',
					ts_finish = '.$this->db->escape($datestart).'
				WHERE
					1 = 1
					AND uid = '.$this->db->escape($this->session->userdata('uid')).'
			');
			if(!isset($frameip) || $frameip == null) {
				$q02 = $this->db->query('
					DELETE FROM task_active
					WHERE
						1 = 1
			...
			if($this->form_validation->run() == false) {
				...
				$q01 = $this->db->query('
					UPDATE task_active SET
						...
						who_ts = '.$this->db->escape($datefinish).',
				...
				if(count($ndEx) > 1) {
					...
					$q01 = $this->db->query('
						UPDATE task_active SET
							...
							redaman_ts =  '.$this->db->escape($datefinish).',
		...
		if($token > 0 || !$this->whitelistEnabled) {
			...
			$q01 = $this->db->query('
				UPDATE task_active SET
					...
					token2_ts = '.$this->db->escape($datefinish).',
	...
	public function retrievePCRFol() {
		date_default_timezone_set('Asia/Jakarta');
			...
			if($q01->num_rows() > 0) {
				...
				$q01 = $this->db->query('
					UPDATE task_active SET
						...
						speed_ts = '.$this->db->escape($datefinish).',
	...
	public function retrieveCloseAuto()	{
		...
		if($token > 0 || !$this->whitelistEnabled) {
			...
			//else if($r01['redaman_passed'] == 0 || $r01['speed_passed'] == 0)
			else{
				$passed = 0;
			}
